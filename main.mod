...
//Partie Stochastique
	function computeSolStochastique()
	{
		...
		var optimumData = new IloOplDataElements();
		optimumData.opti = optimum; //add the optimum value to the model's variables
	...
	function computeSolStochastiqueMTZ()
	{
		...
		var subCplex = new IloCplex();
		var subDataSource = new IloOplDataSource("data/"+ s +".dat");
		var subDataSource2 = new IloOplDataSource("data/"+ s +"_covMatrix.dat");
		
		var optimumData = new IloOplDataElements();
		optimumData.opti = optimumMTZ; //add the optimum value to the model's variables
